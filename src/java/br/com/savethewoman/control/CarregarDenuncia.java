 ...
 * @author ferbe
 */
@WebServlet(name = "CarregarDenuncia", urlPatterns = {"/CarregarDenuncia"})
public class CarregarDenuncia extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            GenericDAO dao = new DenunciaDAOImpl();
            request.setAttribute("Denuncia", dao.carregar(idDenuncia));
            request.getRequestDispatcher("ListarDenuncia").forward(request, response);
        }
        catch(Exception ex){
            System.out.println("Problemas ao carregar denúncia!\n Erro: " +ex.getMessage());
            ex.printStackTrace();
        }
    }
